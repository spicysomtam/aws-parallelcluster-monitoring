...
case "${cfn_node_type}" in
    ...
    log_group_names=$(aws cloudformation describe-stacks --region $cfn_region --stack-name $stack_name --query 'Stacks[*].Parameters[?ParameterKey==`ClusterCWLogGroup`].ParameterValue' --output text)
	...
	ComputeFleet)
		...
		if [[ $compute_instance_type =~ $gpu_instances ]]; then
			...
			/usr/local/bin/docker-compose -f /home/${cfn_cluster_user}/${monitoring_dir_name}/docker-compose/docker-compose.node-exporter.yml -p monitoring-compute up -d
        fi
	;;
	LoginNode)
			/usr/local/bin/docker-compose -f /home/${cfn_cluster_user}/${monitoring_dir_name}/docker-compose/docker-compose.node-exporter.yml -p monitoring-login up -d
	;;
esac
